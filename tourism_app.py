...
# Set dataset path
DATA_PATH = os.path.join(os.getcwd(), "Tourism Dataset")
...
def main():
    ...
    # Prepare inputs
    input_data_reg = np.array([[visit_year, visit_month, attraction_type, continent, country]])
    input_data_class = np.array([[visit_year, visit_month, continent, country]])
    ...
    result_with_recommendations = pd.concat([result_df, recommendations.rename(columns={"Attraction": "Recommended Attraction", "Rating": "Avg Rating"})], axis=1)
    
    csv = result_with_recommendations.to_csv(index=False).encode('utf-8')
    st.download_button("📥 Download Predictions & Recommendations", csv, file_name='tourism_recommendations.csv', mime='text/csv')
